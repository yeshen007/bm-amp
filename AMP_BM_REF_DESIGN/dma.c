...
#define dprintf(...)
//#define dprintf(fmt,arg...) 	printk(fmt,##arg)
...
s32 alt_dma_init(const ALT_DMA_CFG_t * dma_cfg)
{
    ...
    /* 初始化通道信息数组channel_info_array，标记每个通道都未分配 */
    for (i = 0; i < 8; ++i)
    {
	...
	/* 写dma控制器需要使用的系统管理器的ctrl寄存器0xFFD08070 */
...
s32 alt_dma_channel_exec(ALT_DMA_CHANNEL_t channel, ALT_DMA_PROGRAM_t * pgm)
{
   ...
   status = alt_dma_channel_state_get(channel, &state);
	...
	/* 通过写DBGCMD寄存器发送DMAGO指令
     * 这条指令是由管理线程执行，它将channel唤醒执行我们的dma代码
	*/
    alt_write_word(ALT_DMA_DBGCMD_ADDR(ALT_DMASECURE_ADDR), 0);
                                         ...
                                         bool use_acp)
{
    ...
    if (status == ALT_E_SUCCESS) 
        status = alt_dma_program_init(program);	//初始化program结构体  

    if (size != 0) {
		u32 sizeleft;			//还剩多少没传输
		u32 udst = (u32)dst;	//dma传输目的物理地址
		u32 usrc = (u32)src;	//dma传输源物理地址
		...
		/* 在program中构造一条dma代码dmamov sar,usrc,目的是启动dma时将usrc写入sar寄存器*/
        if (status == ALT_E_SUCCESS)
            status = alt_dma_program_DMAMOV(program, ALT_DMA_PROGRAM_REG_SAR, usrc);
        /* 在program中构造一条dma代码dmamov dar,usrc,       目的是启动dma时将udst写入dar寄存器*/
        if (status == ALT_E_SUCCESS)
        ...
        if (usrc & 0x7) {
            u32 aligncount = MIN(8 - (usrc & 0x7), sizeleft);	//需要先传送多少字节后才源地址才8字节对齐
            sizeleft -= aligncount;		
            ...
            if (status == ALT_E_SUCCESS) {
                status = alt_dma_program_DMAMOV(program, ALT_DMA_PROGRAM_REG_CCR,
                                                (   ((aligncount - 1) << 4) // SB	//src_burst_len
                                                  | ALT_DMA_CCR_OPT_SS8				// src_burst_size
                                                  | ALT_DMA_CCR_OPT_SA_DEFAULT		//src_inc
                                                  | ALT_DMA_CCR_OPT_SP_DEFAULT		// src_prot_ctrl
                                                  | arcache							//src_cache_ctrl 
                                                  | ((aligncount - 1) << 18) // DB
                                                  | ALT_DMA_CCR_OPT_DS8
        ...
        u32 burstcount = sizeleft >> 3;		//有多少个一个1len8size的burst

        bool correction = (burstcount != 0);	//至少有一个1len8size的burst

        // Update the size left to transfer
        sizeleft &= 0x7;		//除了burst对齐的还有多少字节
        ...
        if (burstcount >> 4) {
            u32 length16burstcount = burstcount >> 4;	//有多少个16 transfer len 8字节size的burst
            burstcount &= 0xf;	//剩下多少一个transfer len 8字节size的burst
            ...
            while (length16burstcount > 0) {
                ...
                u32 loopcount = MIN(length16burstcount, 256);	//因为loop寄存器只用8位，所以最大256
				...
				/* 因为在ccr中指定了len16 size8，因此这里一个DMALD会加载16*8字节也就是一个16len8size burst */
                if (status == ALT_E_SUCCESS)
		...
		/* 到这里   16len8size的已经传送完，还剩1len         8size和单字节的  */
        if (burstcount)	//还剩1len  8size的burst
        {
            // Program in the following parameters:
            //  - SS64 (Source      burst size of 8-byte)
            //  - DS64 (Destination burst size of 8-byte)
        ...
        // If src and dst are mod-8 congruent, no correction is needed.
		/* 1len8size的基本已经处理完，但有个源和目的地址可能不对齐的情况要处理，
		 * 如果correction != true,说明都没有1len8sizeburst，因此不需要校正，
		 * 如果(usrc & 0x7) == (udst & 0x7)，说明源和目的地址对齐，因此也不需要校正
		 * 校正的结果就是让最后的少于8字节的数据在目的那边八字节对齐
		*/
        if (   (correction == true) && ((usrc & 0x7) != (udst & 0x7)) ) {	
            if (status == ALT_E_SUCCESS) {
		...
		/* 到这里16len8size和1len    8size的burst已经传送完，只剩单字节的   */
        if (sizeleft) {
    ...
    /* 到这一步所有的dma传输指令已经构建完 */
	//如果send_evt为真，还要构建dma发送事件指令，但这里只会触发一个irq，
	//因为之前已经更改INTEN寄存器
    if (send_evt) {
    	  if (status == ALT_E_SUCCESS) {
    ...
    //如果构建最后程序结束的指令不成功则重新初始化program并返回，
    //如果构建成功则到最后一步执行dma程序
    if (status != ALT_E_SUCCESS) {
                                                      ...
                                                      u32 size)
{
    ...
    while (sizeleft > 0)
    {
		...
		printk("DMA[P->M][S]: Creating loop for %u transfer(s).\n", (unsigned)loopcount);
                                               ...
                                               u32 size)
{
...
#endif
    {
		...
		printk("DMA[P->M][fpga2arm]: FIFOs disabled.\n");
                                         ...
                                         ALT_DMA_EVENT_t evt)
{
    ...
    // Execute the program on the given channel.
	printk("fpga2arm alt_dma_channel_exec\n");
    return alt_dma_channel_exec(channel, program);
}
